    ...
    <div className="max-w-5xl mx-auto space-y-8 p-8">
      ...
      <div className="flex items-end justify-between">
        ...
        <div className="text-right">
          ...
          <p className="text-sm font-bold text-primary">#CC-8921: Pine Ridge Delinquency</p>
        </div>
      </div>
              ...
              <div className="bg-slate-50 rounded-xl p-5 border border-slate-200 flex items-center justify-between gap-6">
                ...
                <div className="flex flex-col gap-2 shrink-0">
                  {letterState.status === 'done' ? (
                    ...
                    <button onClick={handleGenerateLetter} disabled={letterState.status === 'generating'}
                      className={`flex items-center gap-2 px-5 py-2.5 text-xs font-bold rounded-xl shadow transition-all ${letterState.status === 'generating' ? 'bg-slate-200 text-slate-400 cursor-not-allowed' : 'bg-primary hover:bg-primary/90 text-white'}`}>
                      <Icons.DocumentGen className="size-4" />
                      {letterState.status === 'generating' ? 'GENERATING...' : 'GENERATE FIRST LETTER'}
                    </button>
                  )}
                  {ledgerState.status === 'done' ? (
                    ...
                    <button onClick={handleGenerateLedger} disabled={ledgerState.status === 'generating'}
                      className={`flex items-center gap-2 px-5 py-2.5 text-xs font-bold rounded-xl shadow transition-all ${ledgerState.status === 'generating' ? 'bg-slate-200 text-slate-400 cursor-not-allowed' : 'bg-slate-700 hover:bg-slate-800 text-white'}`}>
                      <Icons.Reports className="size-4" />
                      {ledgerState.status === 'generating' ? 'BUILDING...' : 'GENERATE LEDGER (.xlsx)'}
                    </button>
                  )}
                </div>
              </div>
          ...
          <div className="mt-8 border-t border-slate-100 pt-8">
            ...
            <div className="bg-primary/5 rounded-xl p-5 border border-primary/10">
              ...
              {groundTruthState.status === 'done' ? (
                ...
                <button onClick={handleGenerateGroundTruth} disabled={groundTruthState.status === 'generating'}
                  className={`flex items-center gap-2 px-5 py-2.5 text-xs font-bold rounded-xl shadow transition-all ${groundTruthState.status === 'generating' ? 'bg-slate-200 text-slate-400 cursor-not-allowed' : 'bg-primary hover:bg-primary/90 text-white'}`}>
                  <Icons.Run className="size-4" />
                  {groundTruthState.status === 'generating' ? 'RUNNING GROUND TRUTH (up to 90s)…' : 'RUN GROUND TRUTH PIPELINE'}
                </button>
              )}
            </div>
          </div>
